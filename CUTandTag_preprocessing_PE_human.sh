...
alignfun (){
    ...
    bowtie2 -t --non-deterministic --mm --phred33 -p $NPROC_PER_SAMPLE --very-sensitive -x hg38 -1 $READ1 -2 $READ2 | samtools sort -@ $NPROC_PER_SAMPLE  -o $WORKING_DIR"/"$samplename".align.bam" &
done
}
...
bcfun (){
	...
	bamCoverage --binSize 10 --smoothLength 30 --normalizeUsing RPGC --effectiveGenomeSize 2913022398 --extendReads 200 -p $3 -b $1 -o $OUT_BW
...
echo -e "Sample\t library_size_dedup\t library_size_with_dup" > $STATS_DIR/library_size.tsv   
...
savefun (){
    ...
    echo -e "${samplename}\t $FIN\t $HQ" >> $STATS_DIR/library_size.tsv   
